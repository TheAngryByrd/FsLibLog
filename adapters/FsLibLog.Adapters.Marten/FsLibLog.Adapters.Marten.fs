...
type FsLibLogLogger(logger: FsLibLog.Types.ILog, ?shouldLogParameters: bool) =
    ...
    let logCommand logF (command: NpgsqlCommand) (ex: exn) =
        let logParams =
            if shouldLogParameters then
                ...
                command.Parameters
                |> Seq.iter (fun p -> parameters.Add(p.ParameterName, p.Value))
    ...
    interface IMartenSessionLogger with
        member __.LogSuccess(command: NpgsqlCommand) = logCommand logger.debug command null


        member __.LogFailure(command: NpgsqlCommand, ex: exn) = logCommand logger.error command ex
