...
module Expect =
    let contains sequence element message =
        ...
        with
        | Some _ -> ()
        | None -> failwithf "%s. Sequence did not contain %A." message element
#endif
...
type TestProvider() as this =
    ...
    member val Contexts: ResizeArray<string * bool * obj> = ResizeArray<string * bool * obj>()
    ...
    interface ILogProvider with
        ...
        member this.OpenNestedContext(message: string) : System.IDisposable = addProp "NDC" message false
...
let tests =
    ...
    <| testList "FsLibLog" [
        testList "getting logger" [
#if !FABLE_COMPILER
            testCase "LogProvider.getLoggerByFunc"
            <| fun _ ->
                let provider = getNewProvider ()
                someFunction ()
                Expect.equal provider.LoggerName "Tests.someFunction" ""
            testCaseAsync "LogProvider.getLoggerByFunc async CE"
            <| async {
                let provider = getNewProvider ()
            ...
            testCase "LogProvider.getLoggerByQuotation"
            <| fun _ -> Expect.equal SomeOtherModule.provider.LoggerName "Tests+SomeOtherModule" ""
#endif
            testCase "LogProvider.getLoggerByName"
            <| fun _ ->
                let provider = getNewProvider ()
        ...
        testList "setting log" [
            ...
            testCase "addParameter operator (>>!)"
            <| fun _ ->
                let provider = getNewProvider ()
                let parameter = "someParmeter"
                let logger = LogProvider.getLoggerByName ">>!"
            ...
            testCase "addContextDestructured"
            <| fun _ ->
                let provider = getNewProvider ()
                let parameter = "someParmeter"
            ...
            testCase "addContextDestructured operator (>>!+)"
            <| fun _ ->
                let provider = getNewProvider ()
                let parameter = "someParmeter"
                let logger = LogProvider.getLoggerByName ">>!+"
            ...
            testCase "openMappedContextDestucturable true"
            <| fun _ ->
            ...
            testCase "openMappedContextDestucturable false"
            <| fun _ ->
                let parameter = "some parameter"
                let provider = getNewProvider ()
            ...
            testCase "openNestedContext false"
            <| fun _ ->
                let parameter = "some parameter"
                let provider = getNewProvider ()
...
#if !FABLE_COMPILER
            testCase "setMessageInterpolated simple"
            <| fun _ ->
            ...
            testCase "setMessageInterpolated object"
            <| fun _ ->
            ...
            testCase "setMessageInterpolated anonymous record"
            <| fun _ ->
